...
if (process.env.NODE_ENV === 'production') {
  app.use((req, res, next) => {
    const hasWww = req.get('Host').match(/^www\..*/i);
    if (req.path.indexOf('/heart-beat') < 0 && req.headers['x-forwarded-proto'] !== 'https' && hasWww === null) {
      return res.redirect(301, ['https://www.', req.get('Host'), req.url].join(''));
    } else if (req.path.indexOf('/heart-beat') < 0 && req.headers['x-forwarded-proto'] !== 'https' && hasWww !== null) {
...
app.use('/heart-beat', (req, res) => {
  console.info('UI SERVER:', 'Health OK');
  res.send('ok');
});
